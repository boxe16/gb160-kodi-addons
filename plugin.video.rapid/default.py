...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.rapid/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.rapid/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to Rapid IPTV" 
			line3 = ('[COLOR white]%s[/COLOR]'%user)
			xbmcgui.Dialog().ok('Rapid IPTV', line1, line2, line3)
			tvguidesetup()
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
...
##########################################
def catchup():
    loginurl   = "http://clientsportals.net:8080/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("rapid", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search Rapid IPTV:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('Rapid IPTV', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('Rapid IPTV', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('Rapid IPTV', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('Rapid IPTV', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('Rapid IPTV', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.rapid/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('Rapid IPTV', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.rapid')
	...
	loginurl   = "http://clientsportals.net:8080/get.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
	EPGurl     = "http://clientsportals.net:8080/xmltv.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('Rapid IPTV','Would You like us to Setup the PVR Guide for You?')
		if dialog:
