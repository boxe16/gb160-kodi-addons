...
def Get():
	...
	m3u  = 'http://helixtv.ninja:83%3A83%2Fget.php%3Fusername%3D'+username+'%26password%3D'+password+'%26type%3Dm3u_plus%26output%3Dts'
	epg  = 'http://helixtv.ninja:83%3A83%2Fxmltv.php%3Fusername%3D'+username+'%26password%3D'+password
	auth = 'http://helixtv.ninja:83/enigma2.php?username='+username+'&password='+password+'&type=get_vod_categories'
	...
	if not auth=="":
		...
		xbmcgui.Dialog().ok('Helix IPTV','[COLOR blue]M3U URL: [/COLOR]%s'%shortm3u,'','[COLOR blue]EPG URL: [/COLOR]%s'%shortepg)
	else:
		return