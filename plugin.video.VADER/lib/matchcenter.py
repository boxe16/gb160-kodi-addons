...
class MyAddon(BaseWindow):
    ...
    def playfromStream(self,item, versionIndex):
        ...
        if embedded == True:
            ...
            base = 'live'
            extension = 'ts'
            tokenString = username + password + str(version['id']) + str(tokenTime)
            ...
            chanUrl = 'http://%s/live/%s/%s/%s.%s' % (apiEndpoint,username, password, version['id'], 'ts')

        li = xbmcgui.ListItem(
            '{title} - {channel}'.format(title=item.getLabel(), channel=version['name'])
        )
        self.player.play(chanUrl, li)
    ...
    def getCachedData(self):
        ...
        try:
            timeNow = time.time()
            url = 'http://{apiEndpoint}/vapi/v2/mc/schedule?username={username}&password={password}&start={start}&end={end}'.format(
                apiEndpoint=apiEndpoint,
                username=username,
                password=password,
            ...
            if len(epgItem['streams']) > 0:
                ...
                if category.lower() in itemCategory.lower() or category.lower() in 'all':
                    ...
                    if embedded == True:
                        ...
                        chanUrl = 'http://%s/live/%s/%s/%s.%s' % (apiEndpoint,username, password, epgItem['streams'][0]['id'], 'ts')
